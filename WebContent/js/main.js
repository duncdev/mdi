...
$(document).ready(function() {
  ...
  $("form").each(function() {
    cs.FormEdit.init(this);
  });

  // Header bar button setup
  $("#menubar button").each(function(index) {
    $(this).data("sequence", index);
  });
  $("#menubar button").click(function() {
    var sectionName = $(this).attr("id");
    if (sectionName.indexOf("_") >= 0)
      sectionName = sectionName.substring(0, sectionName.indexOf("_"));

    var dir = ["right", "left"];
    if (currentSectionName == null) $("#section_" + sectionName).show();
    else {
      // establish direction of transition
      var from = $('#menubar button[id="' + currentSectionName + '"]').data(
        "sequence"
      );
      ...
      $("#section_" + currentSectionName).hide(
        "slide",
        { direction: dir[0] },
        300
      );
    ...
    $('#menubar button[id="' + sectionName + '"]').addClass(
      "headerButtonActive"
    );
    currentSectionName = sectionName;
  });

  $("#file_list_clients").click(function() {
  ...
  $("#file_list_products").click(function() {
    new ViewListProductInstance($(this));
  });
  $("#file_list_salesteams").click(function() {
    new ViewListSalesTeamInstance($(this));
  });

  $("#file_new_client").click(function() {
    new ViewFormClientInstance(0);
  });
  $("#file_new_product").click(function() {
    new ViewFormProductInstance(0);
  });
  $("#file_new_salesteam").click(function() {
    new ViewFormSalesTeamInstance(0);
  });

  $("#window_close_all").click(function() {
    cs.view.closeAll();
  });
